export class Octets {
  ...
  constructor(param?: any, srcStart?: number, length?: number) {
    if (param == null) this.reserve(Octets.DEFAULT_SIZE);
    else if (typeof param === 'number') this.reserve(param);
    else this.replace(param, srcStart, length);
  }
  ...
  public toHexString(): string {
    return Array.from(this.getBytes())
      .map((b) => ('0' + (b & 0xff).toString(16)).toUpperCase().slice(-2))
      .join('');
  }
  ...
  static arrayCopy(src: Uint8Array, srcStart: number, target: Uint8Array, targetStart: number, length: number) {
    for (let i = 0; i < length; i++) {
      target[targetStart + i] = src[srcStart + i];
    }
  }

  static copy(src: Uint8Array, start: number, end?: number) {
