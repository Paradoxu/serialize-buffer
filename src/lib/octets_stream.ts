...
export class OctetsStream extends Octets {
  ...
  public getByteSequence(nBytes: number): Uint8Array {
    ...
    if (this.isLittleEndian) return ret.reverse();
    return ret;
  }
  ...
  public unmarshalLong(): BigInt {
    if (this.pos + 8 > this.size()) throw new Error('MarshalException');
  ...
  public marshalString(value: string, encoding?: 'utf8' | 'utf16le'): OctetsStream {
    ...
    for (const byte of buffer) this.marshalByte(byte);
    return this;
  }
  ...
  public compactUint32(value: number): OctetsStream {
    ...
    if (value < 16384) return this.marshalShort(value | 0x8000);
    if (value < 536870912) return this.marshalInt(value | 0xc0000000);
  ...
  public unmarshalString(length?: number, encoing?: 'utf8' | 'utf16le'): string {
    ...
    if (this.pos + strLength > this.size()) throw new Error('MarshalException - End of stream');

    if (strLength === 0) return '';
    const buffer = Buffer.alloc(strLength);
    for (let i = 0; i < strLength; i++) buffer[i] = this.getByte(this.pos++);
