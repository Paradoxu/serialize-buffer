...
interface MarshalConfigInterface {
     ...
     */
    (configuration: { type: ['String'], compact?: boolean, length?: number, strLength?: number, encoding?: AllowedEncoding }): ReturnAnnotation;
    // (configuration: { type: ['String'], compact?: boolean, length?: number, encoding?: AllowedEncoding }): ReturnAnnotation;
...
function resolveType(type: any, target: any, name: string, configuration?: Partial<MarshalConfiguration>) {
    const properties: Map<string, PropConfiguration> = Reflect.getOwnMetadata('properties', target) || new Map<string, PropConfiguration>();
    const isArray = Array.isArray(type);
    type = isArray ? type[0] : type;

    const isMarshal = Marshal.isPrototypeOf(target) || Marshal.isPrototypeOf(type);
    const isOctets = Octets.isPrototypeOf(target) || Octets.isPrototypeOf(type) || type?.prototype?.constructor?.name === 'Octets';
