import { PropConfiguration } from './metadata';
import { OctetsStream } from './octets_stream';

export abstract class Marshal implements Record<string, any> {
  constructor() {
    const fields = this.getFields();

    for (const field of fields) {
      if (!field.isTypeResolved && field.resolveType) field.resolveType();
    }
  }
      ...
      try {
        if (field.isArray) {
          ...
          if (field.isMarshal) {
            ...
            for (let i = 0; i < length; i++) os[`marshal${field.type}`](this[field.name][i]);
          }
          ...
          if (typeof this[field.name] === 'string') os.marshalInt(enumValues[this[field.name]]);
          else os.marshalInt(this[field.name]);
  ...
  unmarshal(os: OctetsStream): OctetsStream {
    ...
    for (const field of fields) {
      if (field.isArray) {
        ...
        this[field.name] = new Array<typeof field.constructor>(length);
        if (field.isMarshal) {
          for (let i = 0; i < length; i++) {
        ...
        this[field.name] = new (field.constructor as any)();
    ...
    for (const field of fields) {
      if (field.isArray) {
        if (field.isMarshal) {
          ...
          for (const f of this[field.name]) sum += this.getSizeOfBasicField(field);
        }
      } else if (field.isMarshal) {
  ...
  private getSizeOfBasicField(field: PropConfiguration): number {
